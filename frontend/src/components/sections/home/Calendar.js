...
const Calendar = () => {
    ...
    const handleClick = (redirect_obj) => {
        if (redirect_obj) {
            window.location.href = redirect_obj.path; // Replacing history.push with standard navigation
        }
    }
    ...
    const onAppointmentUpdated = (e) => {
        ...
        const data = { id, subject: text, notes: description, created_by: user.id, staff: reminder_for, start_date: startDate, end_date: endDate }
    ...
    const onAppointmentAdded = (e) => {
        ...
        const data = { subject: text, notes: description, created_by: user.id, staff: reminder_for, start_date: startDate, end_date: endDate }
    ...
    const onAppointmentFormOpening = (e) => {
        ...
        let redirect_obj = null;

        if (risks_) {
            redirect_obj = { path: "/riskassessment/add-riskassessment", object: risks_ }
            dispatch(riskSchedulerActions.setSelectedriskScheduler(risks_))
        } else if (plan_) {
            redirect_obj = { path: "/supportplan/add-supportplan", object: plan_ }
            dispatch(planSchedulerActions.setSelectedPlanSchedulers(plan_))
        } else if (accident_) {
            redirect_obj = { path: "/resident/detail", object: accident_ }
            dispatch(accidentSchedulerActions.setSelectedAccidentSchedulers(accident_))
        } else if (evaluation_) {
            redirect_obj = { path: "/evaluation/deatil", object: evaluation_ }
        ...
        const { form } = e;
        form.option('items', [
            { label: { text: 'ID' }, name: 'id', dataField: 'id', disabled: true, editorType: 'dxTextBox' },
            { label: { text: 'Title' }, name: 'description', editorType: 'dxTextBox', dataField: 'text', editorOptions: { onValueChanged(args) { form.updateData('text', args.value); } } },
            { label: { text: 'description' }, name: 'Description', dataField: 'description', editorType: 'dxTextBox', editorOptions: { onValueChanged(args) { form.updateData('description', args.value); } } },
            { editorType: 'dxDateBox', dataField: 'startDate', editorOptions: { width: '100%', type: 'datetime', onValueChanged(args) { form.updateData('startDate', new Date(args.value)); } } },
            { name: 'endDate', dataField: 'endDate', editorType: 'dxDateBox', editorOptions: { width: '100%', type: 'datetime' } },
            { label: { text: 'Staff' }, editorType: 'dxSelectBox', dataField: 'reminder_for', editorOptions: { items: staff, displayExpr: 'email', valueExpr: 'id', onValueChanged(args) { form.updateData('reminder_for', args.value); } } },
            { editorType: 'dxButton', editorOptions: { text: "Click here to evaluate", onClick: _ => handleClick(redirect_obj) } },
        ]);
    }
