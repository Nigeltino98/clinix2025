...
function Addform() {
    ...
    const handleChange = (event) => {
        switch (event.target.name) {
            case 'resident':
                setState({ ...state, resident: event.target.value })
                break;
            case 'assessment':
                setState({ ...state, assessment: event.target.value })
                break;
            case 'next_assement_date':
                setState({ ...state, next_assement_date: event.target.value })
                break;
            case 'discontinue':
                setState({ ...state, discontinue: event.target.checked })
                break;
            default:
        }
    }
    ...
    useEffect(() => {
        getApi(response => {
            dispatch(assessmentActions.setAssessment(response.data));
        }, token, "/api/assessment")
    }, [dispatch, token])
    ...
    const handleSubmit = (event) => {
        ...
        if (form.checkValidity() === false) {
            ...
            if (JSON.stringify(selected_resident) === '{}') {
                ...
                dispatch(residentActions.setSelectedResident(res))
            }
            postApi(response => {
                ...
                <div className="ms-panel">
                    ...
                    <div className="ms-panel-body">
                        <Form noValidate validated={validated} onSubmit={handleSubmit}>
                            ...
                            <Button type="reset" variant="warning" className="mt-4 mr-2">Cancel</Button>
                            <Button type="submit" className="mt-4">Begin Evaluation</Button>
                        </Form>
                    </div>
                </div>
            ) : null}
