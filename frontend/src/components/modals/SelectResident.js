...
const StaffEdit = (props) => {
    ...
    /*
     * Only show active residents in the dropdown.
     *
     * Archived residents:
    ...
    const handleChange = (event) => {
        if (event.target.value === 'all') {
            ...
            const resident = activeResidents.find(
                item => item.national_id === event.target.value
            );

            dispatch(
                residentActions.setSelectedResident(resident)
            );
        }
    };
    ...
    useEffect(() => {
        getApi(
            response => {
                dispatch(homeActions.setHome(response.data));
            },
            token,
            "/api/home"
        );
        ...
        getApi(
            response => {
                dispatch(
                    residentActions.setResidents(response.data)
                );
            },
            token,
            "/api/resident"
